...
namespace TAO.IdentityApp.Web.ValidationRules.FluentValidation
{
    public class SignUpViewModelValidator:AbstractValidator<SignUpViewModel>
    {
...

        {
            ...
            RuleFor(x => x.Password).NotEmpty().Must(IsPasswordValid).Matches(x=>x.PasswordConfirm).WithMessage("Password must be match.");
            RuleFor(x => x.PasswordConfirm).NotEmpty().Must(IsPasswordValid).Matches(x=>x.Password).WithMessage("Password must be match.");
        }
